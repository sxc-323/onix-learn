...
static char *number(char * str,unsigned long num,int base,int size,int precision,int flags)
{
    ...
    // 如果进制基数小于2 或者 大于 36，则退出处理
    // 也即本程序只能处理基数在 2-32 之间的数
    if(base<2 || base > 36)
        return 0;
    
    // 如果 flags 指出要填零，则置字符变量 c='0',否则 c等于空格字符
    c=(flags & ZEROPAD) ? '0' :' ';

    //如果 flags 指出是带符号数并且数值num 小于0,则置符号变量 sign=负号，并使num取绝对值
    if(flags & SIGN && num<0)
    {
        sign='-';
        num=-num;
        ...
        // 否则如果 flags 指出是加号，则置sign=加号，否则若类型带空格则sign=空格，否则置0
    ...
    // 若flags 指出是特殊转换，则对于十六进制宽度再减少2（用于0x）
    if(flags & SPECIAL)
    {
        if (base==16)
            size-=2;
    ...
    // 若 flags 中没有左调整（左对齐）标志，则在放置宽度中存放c 字符（'0' 或空格）
    if(!(flags & LEFT))
        while (size-->0)
            *str++=c;
    ...
    //若数字个数小于精度值，则str中存入（精度值-i)个‘0’
    while (i< precision--)
        *str++='0';

    // 将转换数值换好的数字字符填入str中，共i个
    while (i-->0)
        *str++=tmp[i];
...
int vsprintf(char * buf,const char *fmt, va_list args)
{
    ...
    repeat:
        ...
        else if (*fmt == '*')
        {
            ...
            // 若此时宽度值小于0,则该负数表示其带有标志域 ‘-’ 标志（左对齐）
            if(field_width<0)
            {
                //因此还需在标志变量中添加该标志，并将字段宽度值取为其绝对值
                field_width=-field_width;
                flags|=LEFT;
            }
        }
        
        //取格式转换串的精度域，并放入precision 变量中
        precision=-1;
